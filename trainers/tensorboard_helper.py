...
class TensorboardScalarHelper(TensorboardHelperBase):
    def add_data(self, tb_datapack):
        ...
        for key, val in scalar_dict.items():
            self.tb_writer.add_scalar(key, val, global_step=tb_datapack.iteration)
...
class TensorboardHistogramHelper(TensorboardHelperBase):
    def add_data(self, tb_datapack):
        ...
        for key, val in histogram_dict.items():
            self.tb_writer.add_histogram(key, val, global_step=tb_datapack.iteration)
...
class TensorboardImageHelper(TensorboardHelperBase):
    def add_data(self, tb_datapack):
        ...
        for key, val in image_dict.items():
            self.tb_writer.add_image(key, val, global_step=tb_datapack.iteration)
...
class TensorboardTextHelper(TensorboardHelperBase):
    def add_data(self, tb_datapack):
        ...
        for key, val in text_dict.items():
            self.tb_writer.add_text(key, val, global_step=tb_datapack.iteration)
...
class TensorboardPusher():
    def __init__(self, opt):
        ...
        self.helper_list = [scalar_helper, histogram_helper, image_helper, text_helper]
